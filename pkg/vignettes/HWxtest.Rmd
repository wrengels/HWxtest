...
Published data with genotype frequencies are often available in the *GenePop* format. These data can be easily converted to *genind* objects and then fed to `hwx.test`. For example, `r citet(olsen2014)` provide a supplemental file containing genotype counts at 15 loci in 12 populations of sea urchins. We can pull those numbers in directly from the online source and analyze them as follows:
...
hwx.test(counts, statName="Chisq", histobins=T, histobounds=c(50, 250), B=1e6)
```
...
There are other test statistics that might be added to `HWxtest` in future versions. For example `r citet(ward2014)` argue that the root-mean-square is a useful measure of departure from Hardy-Weinberg proportions.
